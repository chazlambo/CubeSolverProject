...
**Working:** full autonomous scan → solve → execute cycle, driven from the
panel menu: the five Modes (Scramble Solve, Idle, Demo, Step Solve, Patterns),
on-panel tuning with defaults in source and overrides in EEPROM, a hardware jog
page, live sensor diagnostics, an EEPROM fault log and lifetime stats. The
display stays live during scan and solve, the abort chord works throughout,
encoder faults are detected and handled, a scan is validated piece-by-piece
with automatic repair, and color classification carries a per-sensor
confidence.
...
| Sketch | Purpose |
|---|---|
| `Test Code/Bench_Tune` | Headless Serial protocol for scripted tuning: moves, parameters, every encoder before/after each move. Driven by `Code/tools/bench.py` |
| `Test Code/Actuator_Test` | Serial menu for manual moves and jam-recovery testing |
| `Test Code/Test_Motor_Calibrate` | Motor encoder calibration (also Settings → Calibration → Motor Positions) |
| `Test Code/Test_Color_Calibrate` | Color calibration (also Settings → Calibration → Color Sensors) |
...
It runs no scan, solve or calibration. It does move real hardware from three
places: **Load** / **Eject**, the **Actuators** page (every servo and ring
position, each face motor and both whole-cube rotations, one part at a time),
and Diagnostics → Tuning → Servos, which drives a horn to each value as you
dial it. Everything under `Screens` is drawing only.
...
Arduino *sketchbook libraries* folder, and the build must compile the repo's
copy, not a stale one. Two working setups:

1. **Point the sketchbook at the repo:** set Arduino's sketchbook location to
   the `Code/` directory (File → Preferences → Sketchbook location). Then
   `#include <CubeSystem.h>` and `#include <kociemba.h>` resolve, and
   `lv_conf.h` is found automatically.
2. **Link from the default sketchbook** (what this machine actually does —
   the sketchbook stays at `Documents/Arduino` so Library Manager installs
   land in the usual place):
   - `Documents/Arduino/libraries/CubeSolver` → symlink to
     `Code/libraries/CubeSolver`
   - `Documents/Arduino/libraries/kociemba` → junction to
     `Code/libraries/kociemba`
   - `Documents/Arduino/libraries/lv_conf.h` → a one-line forwarder header
     that does `#include "../../Projects/Cube_Solver/Code/libraries/lv_conf.h"`

Either way, do **not** copy the libraries into the sketchbook — you'll edit
the repo and build a stale copy. And never place an `lv_conf.h` inside the
`lvgl` library folder itself: if lvgl's own directory ever lands on the
include path, `__has_include("lv_conf.h")` silently builds the whole project
against that copy instead of the repo's.
...
- **The recolorable masks are RGB565A8, not A8.** LVGL reads an uncompressed
  ...
  first. The frame band alone is 49 KB — bigger than the 32 KB pool of the
  time, and most of today's 64 KB — so as A8 it fails to allocate, and with
  `LV_USE_LOG` at 0 it fails *silently*, drawing nothing at all. This costs
  about 220 KB of flash and is worth it.
...
comments in that file for the load-bearing settings.

**`LV_MEM_SIZE` is 64 KB, and running out of it hangs the machine.**
...
`LV_USE_LOG` is 0. It started at 32 KB, which the themed menu alone came within
~1 KB of; the operation screens took the peak to ~27 KB and the firmware hung
at boot with no output at all, so it went to 48 KB. The Sensor Test chips then
took the widget set to 35 KB and tripped the boot warning below, so it went to
64 KB. Every raise was set against a measurement, not a guess.
...
- **The color scanner is mounted rotated 90° from its original layout.** It
  was turned in CAD to make the assembly fit, which changed which faces each
  scan pass reads — from `[L,B] [U,F] [D,R]` to
...
Menu → **Settings → Calibration → Motor Positions**, or flash
`Test Code/Test_Motor_Calibrate`.
...
Menu → **Settings → Calibration → Color Sensors**, or flash
`Test Code/Test_Color_Calibrate`.
...
2. Turn the wheel to **Load & Scan Cube**, press SELECT.
3. Insert a scrambled cube, press SELECT. It scans (~25–40 s) and the main
   menu becomes **Cube Ready**.
4. Select **Solve**, press SELECT. It computes and reports the move count.
5. Press SELECT to execute (LEFT cancels and leaves the cube as it is).
...
| Scan 91 | Motors failed to home before the first clamp (jam / encoder fault) |
...
- **The panel is driven synchronously, on purpose.** `CUBE_DISPLAY_ASYNC_DMA`
  in `CubeDisplay.h` is 0: every LVGL flush is written straight to the glass
  and returns when it is on the wire. The driver's async DMA/differential mode
  (mode 1) is faster, but on this board it left the glass holding the previous
  frame in large chunks, or coloured snow, while the driver believed the new
  one had been sent — and because that mode only ever re-sends pixels it thinks
  changed, the damage stayed until something drew over it. The header comment
  has the history. `cubeDisplay.repaintAll()` re-sends every pixel; the sketch
